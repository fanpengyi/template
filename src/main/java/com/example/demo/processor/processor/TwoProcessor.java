...
@Service
@Slf4j
public class TwoProcessor extends AbstractTemplateProcessor<String> {
    ...
    protected Response<Void> doProcess(String useId) {
        log.info("执行 TwoProcessor 的逻辑"+useId);
        DataHolder.get().getResultMaps().put("Two","TwoProcessor");
